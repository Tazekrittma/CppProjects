...
class Distributeur
{
	private:
		...
		unsigned nb;
		//Boisson * stock[N];
		//Boisson ** stock = new Boisson[N];
		Boisson ** stock;
		unsigned * quantite;
	public:
		Distributeur(String mod,unsigned capacite);
		~Distributeur();
...
Distributeur::Distributeur(String mod, unsigned capacite)
{
	strcpy(modele,mod);
	nb=0;

	stock= new Boisson * [capacite];
	quantite= new unsigned[capacite];
...
*/
	...
	//Distributeur
	Distributeur dis("A400", 15);  // test d'ajout d'une boisson
	...
	BoissonChaude *havae = new BoissonChaude("havae","2/12/1963",200,3,32,false); // test de boisson chaude
	BoissonChaude *moka= new BoissonChaude("moka","05/02/1998",20,8,45,false);
	havae->afficher(true);
	moka->afficher(true);
